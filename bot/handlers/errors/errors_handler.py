...
async def errors_handler(update, exception):
    ...
    await Update.get_current().message.reply(_('Ошибка: {}').format(exception))
    ...
    if isinstance(exception, CantParseEntities):
        await Update.get_current().message.answer(_("Попало в эррор хендлер. CantParseEntities: {}").format(exception.args))
        return True
...
def register_errors_handler(dp: Dispatcher, i18n: ACLMiddleware):
    global _
    _ = i18n.gettext
